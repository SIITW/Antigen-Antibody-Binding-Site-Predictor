...
def triangulate_unrefined(ppi, config):
    ...
    print("now triangulate the unrefined pdb document")
...
def preprocess(processed_ppi, config):
    ...
    for ppi in processed_ppi:
        ...
        out_grid_antigen = config['dirs']['grid'] + '/' + antigen_ppi + '.npy'
        out_grid_antibody = config['dirs']['grid'] + '/' + antibody_ppi + '.npy'
        if os.path.exists(out_grid_antigen) and os.path.exists(out_grid_antibody):
            print(f"{out_grid_antigen,out_grid_antibody} already exists...")
            continue

        try:
            crop_pdb_one(ppi, config, use_refined=False)
        except:
            raise ValueError("step crop has failed,please check the crop code")
        try:
            triangulate_unrefined(ppi, config)
        except:
            raise ValueError("step triangulate has failed,please check the triangulate code")
        try:
            compute_patches([ppi], config,overwrite=True)
        except:
            raise ValueError("the step compute patches has failed,please check the correspondingly code")

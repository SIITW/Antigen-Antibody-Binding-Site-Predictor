...
def compute_dssp(ppi, config):
    ...
    # remove hydrogens
    remove_comments(antigen_pdb_path, antigen_pdb_tmp_path)
    ...
    antigen_dssp = DSSP(antigen_model, antigen_pdb_tmp_path, dssp='mkdssp')  # example of a key: ('A', (' ', 1147, ' '))
...
def convert_one_patch(ppi, config):
    ...
    antigen_out_dssp = config['dirs']['patches'] + antigen_pid+'/'+'{}_{}_dssp_features.npy'.format(antigen_pid, antigen_ch)
    antibody_out_dssp = config['dirs']['patches'] + antibody_pid + '/' + '{}_{}_dssp_features.npy'.format(antibody_pid,antibody_ch)
    radius = config['ppi_const']['patch_r']

    p1_rho, p1_theta, p1_input_feat, p1_resnames, p1_coord_3d = read_patch(antigen_pid, antigen_ch, config)
    ...
    p1_coord_grid = compute_patch_grid(p1target_x, p1target_y, p1_coord_3d, radius,interpolate=False)
    p2_coord_grid = compute_patch_grid(p2_x, p2_y, p2_coord_3d, radius,interpolate=False)
    dist_grid = np.sqrt(np.sum(np.square(p1_coord_grid - p2_coord_grid), axis=-1))
    print(f"Average distance between surfaces: {dist_grid.mean()}")
    # dimension 5 + 5 + 1
    antigen_single_grid = np.concatenate([p1target_patch_grid, np.expand_dims(dist_grid, axis=-1)], axis=-1)
    antibody_single_grid = np.concatenate([p2_patch_grid, np.expand_dims(dist_grid, axis=-1)], axis=-1)
    ...
    antigen_single_grid = np.concatenate([antigen_single_grid, p1_coord_grid], axis=-1)
    antibody_single_grid = np.concatenate([antibody_single_grid, p2_coord_grid], axis=-1)
    ...
    # compute dssp of all
    antigen_dssp = convert_dssp_to_feat_all(antigen_dssp,p1_resnames)
    antibody_dssp = convert_dssp_to_feat_all(antibody_dssp,p2_resnames)
    ...
    np.save(antigen_out_dssp, antigen_dssp)
    np.save(antibody_out_dssp, antibody_dssp)
    return None
...
def convert_to_images(ppi_list, config):
    for ppi in tqdm(ppi_list):
        ...
        if os.path.exists(antigen_out_grid) and os.path.exists(antibody_out_grid):

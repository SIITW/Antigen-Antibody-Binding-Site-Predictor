...
def get_pdb_features(pdb_file):
    ...
    for line in atom_lines:
        ...
        amino_acid = amino_acid_codes.get(residue_name, 'X')  # Use 'X' for unknown or non-standard amino acids
        feature = (atom_serial, atom_coords, amino_acid)
...
def generate_features(name, device):
    ...
    for i in range(len(list)-1):
        ...
        if seq_name[0] == '>':
            ...
            # 设置节点特征
            # 点特征
            graph.ndata['pos'] = points.clone().detach()
            graph.ndata['f'] = features.clone().detach()

...
from se3.equivariant_attention.from_se3cnn import utils_steerable
from se3.equivariant_attention.fibers import Fiber, fiber2head
from se3.utils.utils_logging import log_gradient_norm
...
def get_basis_and_r(G, max_degree, compute_gradients=False):
    ...
    """
    #basis
    #第一个维度（ind_radius）存储球坐标系中的半径值。
    #第二个维度（ind_alpha）存储球坐标系中的方位角值。
    #第三个维度（ind_beta）存储球坐标系中的极角值。
    ...
    def forward(self, features, **kwargs):
        ...
        for k, v in features.items():
            if str(k) in self.transform.keys():
                # transform random tensor 8*39(trans) dot 929*39*1 -> 929,8,1
...
class GMABSE3(nn.Module):
    ...
    def forward(self, v, k: Dict=None, q: Dict=None, G=None, **kwargs):
        ...
        with G.local_scope():
            ...
            # 边特征 k 和节点特征 q 之间的内积 存入G.edata['e']
            ...
            # Perform attention-weighted message-passing  attention value*V

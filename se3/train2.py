...
def drawpic(reducer, embedding_list, label_list, fold, count):
    ...
    plt.savefig('./pic/esmonly_tsne_'+str(fold)+'_' + str(count) + '.jpg')  # 显示图形
    plt.clf()
...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.mlp_for_se3 = Seq(Lin(128, 1280))
...
def model_test(fold):
    ...
    with torch.no_grad():
        ...
        reducer = manifold.TSNE(n_components=2,perplexity=13,init='pca',random_state=501)
    ...
    with open('result/esmonly_(' + str(fold) + ').txt', 'w') as f:
...
for fold in [0, 1, 2, 3, 4]:
    ...
    patience = 7
    # todo change name
    checkpoint = 'checkpoint/esmonly_' + str(fold) + '.pt'
